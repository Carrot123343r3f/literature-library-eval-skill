...
`collect_open_sources.ONLINE_OPERATION_PERMISSIONS`.  It resolves entry-point,
network, path, and delivery semantics; other documentation must link here
rather than restating a conflicting default.

| User intent | Required entry point | Delivery | Formal sufficiency claim? |
| --- | --- | --- | --- |
| Evaluate an existing library | `run_audit.py --run-config ...` | A–F diagnostic; gaps are `not_assessable` | No, unless evidence supports it |
