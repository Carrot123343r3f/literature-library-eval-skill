...
审计范围：入口协议、`run-config` 契约、范围路由、A–F 评估、外部检索、候选筛选、优化模块、输出归档、恢复执行和文档/示例一致性。

## 发布门禁元数据

| 项目 | 当前值 |
| --- | --- |
| 适用版本/提交 | 维护者在发布或复核时填写；不可将本记录视作浮动版本的自动证明 |
| 最近复核日期 | 2026-07-28 |
| 未决风险 | 外部来源适配器应以录制响应/契约 fixture 持续验证；真实服务返回结构变化时必须复核 |
| 下次触发条件 | 修改入口、权限、schema、外部来源适配器、报告渲染或新增指标时；以及每次发布前 |
| 必跑门禁 | `python scripts/check_consistency.py --strict`；`python -m pytest -m contract_security -q`；`python -m pytest -q`；示例产物再生成与语义断言验证 |
