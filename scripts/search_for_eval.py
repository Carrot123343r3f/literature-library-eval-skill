...
Dev/validation set separation（v2）：
  --dev-set 指定开发集（用于迭代反馈），最终冻结查询后使用 --frozen-query --validation-set 进行 A2 判定。
...
def resolve_query_inputs(ctx, run_config=None):
    """Resolve keywords and PICO from either context.json or run-config."""
    ctx = ctx or {}
    run_config = run_config or {}
    keywords = ctx.get('keywords', []) or ([ctx.get('research_question')] if ctx.get('research_question') else [])
    if not keywords:
        research_question = (run_config.get('project', {}) or {}).get('research_question')
        if research_question:
            keywords = [research_question]
    pico = None
    decomp = (ctx.get('search_decomposition') or {}).get('pico', {})
    if isinstance(decomp, dict) and decomp:
        pico = {}
        for key in ('object', 'technology', 'performance', 'context', 'context_field'):
            value = decomp.get(key)
            if isinstance(value, dict) and value.get('value'):
                pico['context' if key == 'context_field' else key] = {'term': value['value']}
    return keywords, pico
...
def main():
    ...
    p.add_argument('--frozen-query', action='store_true',
                   help='声明当前查询已冻结；只有此模式才允许读取非 provisional validation-set')
    ...
    p.add_argument('--allow-partial', action='store_true',
                   help='允许来源请求失败后以部分结果退出；默认失败请求返回非零状态')
    a = p.parse_args()
    ...
    resolved_keywords, resolved_pico = resolve_query_inputs(ctx, rc)
    ...
    if not queries:
        p.error('无法从 context/run-config 构造检索式；请提供 keywords、research_question、PICO 或 --initial-query')
    if a.validation_set and not a.ai_provisional and not a.frozen_query:
        p.error('--validation-set 只能在 --frozen-query 下读取；首轮诊断请使用 --ai-provisional')
    if a.frozen_query and len(queries) != 1:
        p.error('--frozen-query requires exactly one final query; remove refinement variants')
    ...
    if unsupported:
        p.error(f'不支持的来源: {", ".join(unsupported)}；当前支持 openalex,crossref,arxiv,europepmc')
    if not sources:
        p.error('至少需要一个受支持的开放来源')
    ...
    failed_requests = [row for row in queries_record if row.get('status') == 'failed']
    if failed_requests and not a.allow_partial:
        print(f'来源请求失败 {len(failed_requests)} 次；已写入 search_meta.json。若要继续使用部分结果，请显式传入 --allow-partial。', file=sys.stderr)
        raise SystemExit(2)
