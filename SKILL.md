...
读取 `references/confirmation-template.md`、`references/autonomous-run.md`、`references/engineering-profiles.md`、`references/coverage-model.md`、`references/indicator-dictionary.md`、`references/engineering-standards.md`、`references/noncoverage-model.md`，以及与问题对应的检索说明。执行细则见 `AI_GUIDE.md`。保存查询计划、来源快照、筛选/去重/版本决策、失败日志和证据状态。绝不要求用户在提示词中提交 API key；只使用现有 MCP、环境变量或用户已配置的合法连接。
...
- `references/noncoverage-model.md`：B–F 入口指针；子项权威定义见 `indicator-dictionary.md`，阈值见 `engineering-standards.md`。
