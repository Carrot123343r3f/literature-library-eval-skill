...
如果你已有 IEEE、Scopus 或 WoS 的导出文件，请使用[离线机构数据库导出样例](examples/institutional-export-offline/README.md)。它包含模拟导出、逐来源 manifest、生成的审计结果和一键复跑验收，并明确说明 A3 覆盖下界与后续具体任务的边界。
...
2. 接受你的文献库导出（JSON/CSV/RIS/BibTeX），或先设计检索策略
3. 在获得所需联网授权后执行检索；在具备所需证据时计算指标并生成审计包

首次默认流程只会生成范围引导页、检索准备计划或文献库健康检查；不会伪造充分性结论。
...
元数据补齐、检索式迭代验证、双库优化和单篇文献证据评价均为独立的按需模块。先完成基础首轮流程，再在确有需要时启用，详见[可选模块说明](docs/optional-modules.md)。
...
> **产品边界**：这是文献库与检索证据审计器，不是自动完成系统综述的引擎。内置联网收集器只做诊断性首轮发现；多轮检索、订阅数据库、跨库语法转换、引文追踪与最终筛选，仍需要显式启用模块并保留 AI 或人工执行记录。路线图中的适配器不代表当前能力。
...
- **阈值是信号，不是判决。** 所有默认值附有依据说明；所有值可被用户覆盖。默认阈值不是已经跨领域校准的常数；跨领域校准需要版本化基准集与分 profile 验证。
